...
public class ImagesJob extends Job {
    ...
    public static final Parcelable.Creator<ImagesJob> CREATOR = new Parcelable.Creator<ImagesJob>() {
    ...
    protected ImagesJob(Parcel parcel) {
        ...
        final int borderTextSize = parcel.readInt();
    ...
    public ImagesJob(long jobId, Product product, int orderQuantity, HashMap<String, String> optionMap, List<?> objectList, int offset,
                     int length, boolean nullObjectsAreBlankPages, int shippingClass) {
        ...
        if (objectList != null) {
            ...
            for (int objectIndex = offset; objectIndex < objectList.size() && objectIndex < (offset + length); objectIndex++) {
                final Object object = objectList.get(objectIndex);
    ...
    public ImagesJob(Product product, int orderQuantity, HashMap<String, String> optionMap, List<?> objectList, int offset, int length,
                     boolean nullObjectsAreBlankPages, int shippingClass) {
    ...
    public ImagesJob(long jobId, Product product, int orderQuantity, HashMap<String, String> optionMap, List<?> objectList, int offset,
        ...
        this(jobId, product, orderQuantity, optionMap, objectList, offset, objectList != null ? objectList.size() : 0,
                nullObjectsAreBlankPages, shippingClass);
    }

    public ImagesJob(Product product, int orderQuantity, HashMap<String, String> optionMap, List<?> objectList, int offset, boolean
            nullObjectsAreBlankPages, int shippingClass) {

        this(0, product, orderQuantity, optionMap, objectList, offset, nullObjectsAreBlankPages, shippingClass);
    }

    public ImagesJob(Product product, int orderQuantity, HashMap<String, String> optionsMap, List<?> objectList, boolean
    ...
    public BigDecimal getCost(String currencyCode) {
        ...
        final BigDecimal sheetCost = product.getCost(currencyCode);
        final int expectedQuantity = product.getQuantityPerSheet();

        final int numOrders = (int) Math.floor((getQuantity() + expectedQuantity - 1) / expectedQuantity);
    ...
    JSONObject getJSONRepresentation() {
        ...
        try {
            ...
            final JSONObject productOptionsJSONObject = addProductOptions(jsonObject);
     ...
     *
     *****************************************************/
    protected void putAssetsJSON(List<UploadableImage> uploadableImageList, JSONObject jsonObject, boolean useNullForBlank) throws
            JSONException {
        ...
        for (UploadableImage uploadableImage : uploadableImageList) {
            if (uploadableImage != null) {
                assetsJSONArray.put(String.valueOf(uploadableImage.getUploadedAssetId()));
            } else if (useNullForBlank) {
                assetsJSONArray.put(JSONObject.NULL);
            }
        }
    ...
    private void addBorderTextList(JSONObject jsonObject) {
        ...
        final JSONArray borderTextJSONArray = new JSONArray();
    ...
    public boolean equals(Object otherObject) {
        ...
        if (otherObject == null || (!(otherObject instanceof ImagesJob))) {
            return false;
        }

        final ImagesJob otherAssetListJob = (ImagesJob) otherObject;
        final List<UploadableImage> otherUploadableImageList = otherAssetListJob.mUploadableImageList;

        if (!UploadableImage.areBothNullOrEqual(mUploadableImageList, otherUploadableImageList)) {
            return false;
        }
